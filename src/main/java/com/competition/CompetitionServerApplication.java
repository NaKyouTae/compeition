...

			{
				...
				if(menus.size() == 0) {
					...
					menu.saveAll(Arrays.asList(
						...
						Menu.builder().idx(UUIDUtil.randomString()).menuOrder(9).level(2).insertDate(DateUtil.now()).parent(m_admin).roleIdx(r_admin.getIdx()).roleTitle(r_admin.getRoleName()).child(Boolean.FALSE).title("Logs").menuGroup("log").url("/admin/log").build()
					));
				}
			}
		};
